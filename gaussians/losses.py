...
def l1_loss(network_output, gt, mouth_mask=None):
    if mouth_mask is not None:
        return torch.abs((network_output - gt) * mouth_mask).mean()
    else:
...
def l1_image_grad_loss(fake, target):
    fake_diff_x = fake[:, :, 1:] - fake[:, :, :-1]
    fake_diff_y = fake[:, 1:, :] - fake[:, :-1, :]

    target_diff_x = target[:, :, 1:] - target[:, :, :-1]
    target_diff_y = target[:, 1:, :] - target[:, :-1, :]

    loss = l1_loss(fake_diff_x, target_diff_x)
    loss += l1_loss(fake_diff_y, target_diff_y)
